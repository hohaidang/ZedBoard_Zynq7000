...
/*!
 * @brief This is constructor used for assign function pointer of read, write
 * and delay
 *
 ...
 *
 */
bme_sensor_handler::bme_sensor_handler(read_fnc user_read, write_fnc user_write,
...
/*!
 * @brief This function is used for initial sensor data, soft reset and get
 * calibration data
 *
 * @param: None
 *
...
BME280_Stat bme_sensor_handler::get_calib_data() {
  ...
  getRegData(BME280_TEMP_PRESS_CALIB_DATA_ADDR, calib_data,
             BME280_TEMP_PRESS_CALIB_DATA_LEN);
  parse_temp_calib_data(calib_data);
  getRegData(BME280_HUMIDITY_CALIB_DATA_ADDR, calib_data,
...
/*!
 * @brief This function is used to parse calibration data read from sensor to
 * struct
 *
...
void bme_sensor_handler::parse_humidity_calib_data(const u8 *reg_data) {
  ...
  dev_.calib_data.dig_h2 =
      (int16_t)BME280_CONCAT_BYTES(reg_data[1], reg_data[0]);
...
/*!
 * @brief This function is used to get sensor data, data get will be store at
 * data_
 *
 * @param: None
 *
...
#ifndef __ZYNQ__
  std::cout << std::fixed << std::setprecision(2) << temp << " deg C, " << pres
            << " hPa, " << hum << " % \n";
#else
  /* xil_printf does not support print float number*/
  int whole_temp, thousandths_temp, whole_pres, thousandths_pres, whole_hum,
      thousandths_hum;
  whole_temp = temp;
  whole_pres = pres;
  whole_hum = hum;
  thousandths_temp = (temp - whole_temp) * 1000;
  thousandths_pres = (pres - whole_pres) * 1000;
  thousandths_hum = (hum - whole_hum) * 1000;
  xil_printf("%d.%3d deg C, %d.%3d hPa, %d.%3d percents of humidity\r\n",
             whole_temp, thousandths_temp, whole_pres, thousandths_pres,
             whole_hum, thousandths_hum);
#endif
#endif
}
 ...
 *
 */
BME280_Stat bme_sensor_handler::set_sensor_mode(const u8 sensorMode) {
  u8 currentMode = BME280_SLEEP_MODE;
  u8 ctrl_means_reg = 0x00;
  BME280_Stat ret = SENSOR_OK;
  /* get ctrl means reg */
  ret = getRegData(BME280_CTRL_MEAS_ADDR, &ctrl_means_reg, 1);
  currentMode = BME280_GET_BITS(ctrl_means_reg, BME280_SENSOR_MODE_MSK,
...
BME280_Stat bme_sensor_handler::put_device_to_sleep(u8 &ctrl_means_reg) {
  u8 reg_set = BME280_SET_BITS(ctrl_means_reg, BME280_SLEEP_MODE,
                               BME280_SENSOR_MODE_MSK, BME280_SENSOR_MODE_POS);
  return setRegData(BME280_CTRL_MEAS_ADDR, &reg_set, 1);
}

BME280_Stat bme_sensor_handler::get_status() { return dev_.status; }
 ...
 *
 */
BME280_Stat bme_sensor_handler::write_power_mode(const u8 mode_set,
                                                 u8 &ctrl_means_reg) {
  ctrl_means_reg = BME280_SET_BITS(
      ctrl_means_reg, mode_set, BME280_SENSOR_MODE_MSK, BME280_SENSOR_MODE_POS);
 ...
 *
 */
BME280_Stat bme_sensor_handler::get_sensor_mode(u8 &sensor_mode) {
  u8 ctrl_means_reg = 0x00;
  BME280_Stat ret = SENSOR_OK;
  ret = getRegData(BME280_CTRL_MEAS_ADDR, &ctrl_means_reg, 1);
  sensor_mode = BME280_GET_BITS(ctrl_means_reg, BME280_SENSOR_MODE_MSK,
                                BME280_SENSOR_MODE_POS);
  return ret;
}

/*!
 * @brief This function is used to set sensor settings. Included osr_t, osr_h,
 * osr_p, stand_by and filter
 *
 * @param[in] desired_settings:
 * @return BME280_Stat
 *
 */
BME280_Stat bme_sensor_handler::set_sensor_settings(
    const bme280_settings &desired_settings) {
  u8 ctrl_means_reg = 0x00;
  BME280_Stat ret = SENSOR_OK;
  /* get ctrl means reg */
  ret = getRegData(BME280_CTRL_MEAS_ADDR, &ctrl_means_reg, 1);
  if (BME280_SLEEP_MODE != BME280_GET_BITS(ctrl_means_reg,
  ...
  if (SENSOR_OK == ret) {
    ...
    if (desired_settings.osr_t != dev_.settings.osr_t ||
        desired_settings.osr_p != dev_.settings.osr_p) {
 ...
 *
 */
void bme_sensor_handler::set_osr_temp_pres_settings(
    u8 &cur_ctrl_means_reg, const bme280_settings settings) {
  cur_ctrl_means_reg =
      BME280_SET_BITS(cur_ctrl_means_reg, settings.osr_t,
                      BME280_CTRL_TEMPERATURE_MSK, BME280_CTRL_TEMPERATURE_POS);
  cur_ctrl_means_reg =
      BME280_SET_BITS(cur_ctrl_means_reg, settings.osr_p,
                      BME280_CTRL_PRESSURE_MSK, BME280_CTRL_PRESSURE_POS);
...
/*!
 * @brief This function is used to get sensor settings. Included osr, filter and
 * stand_by
 *
 ...
 */
void bme_sensor_handler::parse_sensor_data(u8 *reg_data,
 ...
 *
 */
void bme_sensor_handler::parse_device_settings(const u8 *reg_data,
                                               bme280_settings &settings) {
  settings.osr_h =
      BME280_GET_BITS(reg_data[0], BME280_CTRL_HUM_MSK, BME280_CTRL_HUM_POS);
  settings.osr_p = BME280_GET_BITS(reg_data[2], BME280_CTRL_PRESSURE_MSK,
                                   BME280_CTRL_PRESSURE_POS);
  settings.osr_t = BME280_GET_BITS(reg_data[2], BME280_CTRL_TEMPERATURE_MSK,
                                   BME280_CTRL_TEMPERATURE_POS);
  settings.filter =
      BME280_GET_BITS(reg_data[3], BME280_FILTER_MSK, BME280_FILTER_POS);
  settings.standby_time =
      BME280_GET_BITS(reg_data[3], BME280_STANDBY_MSK, BME280_STANDBY_POS);
}
 ...
 *
 */
BME280_Stat bme_sensor_handler::getRegData(u8 regAddr, u8 *regData,
                                           const u32 len) {
 ...
 *
 */
BME280_Stat bme_sensor_handler::setRegData(u8 regAddr, const u8 *setData,
                                           const u32 len) {
 ...
 */
double bme_sensor_handler::compensate_temperature(
  ...
  var1 = static_cast<double>(uncomp_data.temperature) / 16384.0 -
         static_cast<double>(dev_.calib_data.dig_t1) / 1024.0;
  var1 = var1 * static_cast<double>(dev_.calib_data.dig_t2);
  var2 = (static_cast<double>(uncomp_data.temperature) / 131072.0 -
          static_cast<double>(dev_.calib_data.dig_t1) / 8192.0);
 ...
 */
double
bme_sensor_handler::compensate_pressure(const bme280_uncomp_data &uncomp_data) {
  double var1;
  double var2;
  double var3;
  double pressure;
  ...
  if (var1 > (0.0)) {
    ...
    var1 = static_cast<double>(dev_.calib_data.dig_p9) * pressure * pressure /
           2147483648.0;
    var2 = pressure * static_cast<double>(dev_.calib_data.dig_p8) / 32768.0;
    pressure =
        pressure +
        (var1 + var2 + static_cast<double>(dev_.calib_data.dig_p7)) / 16.0;
 ...
 */
double
bme_sensor_handler::compensate_humidity(const bme280_uncomp_data &uncomp_data) {
  double humidity{0.0};
  ...
  var1 = static_cast<double>(dev_.calib_data.t_fine) - 76800.0;
  var2 = (static_cast<double>(dev_.calib_data.dig_h4) * 64.0 +
          (static_cast<double>(dev_.calib_data.dig_h5) / 16384.0) * var1);
  var3 = uncomp_data.humidity - var2;
  var4 = static_cast<double>(dev_.calib_data.dig_h2) / 65536.0;
  var5 =
      (1.0 + (static_cast<double>(dev_.calib_data.dig_h3) / 67108864.0) * var1);
  var6 = 1.0 + (static_cast<double>(dev_.calib_data.dig_h6) / 67108864.0) *
                   var1 * var5;
  var6 = var3 * var4 * (var5 * var6);
  humidity = var6 * (1.0 - static_cast<double>(dev_.calib_data.dig_h1) * var6 /
                               524288.0);
